...
class ConsensusModuleAgent implements Agent, MemberStatusListener
{
    ...
    @SuppressWarnings("unused")
    public void onAddMember(final long correlationId, final int memberId, final String endpointsDetail)
    {
        if (null == election && Cluster.Role.LEADER == role)
        {
            final ClusterMember member = ClusterMember.parseEndpoints(memberId, endpointsDetail);
